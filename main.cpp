...
private:
    ...
    int di[100001], nod_d; //vect de dst pt darb + nodint rezultat_euler[MAX]; //euler
    ...
    int e; // cuplaj
    vector<int> lis[MAX]; // listele de adiacenta pt cuplaj
    bitset<MAX> vizi_cuplaj; //cuplaj
    int stanga[MAX], dreapta[MAX], rasp_cuplaj; //cuplaj
    //BFS - declarari
    ...
    //CTC - declarari
    //int N, M;
    int ctck=0, viz[100005], viz_t[100005];
...
bool graf :: cupleaza(int t)
{
    ...
    for(int i : lis[t])
    {
        if(dreapta[i] == 0) //daca gasim un nod adiacent fara cuplaj
        {
            stanga[t] = i; // cupl t si i
            dreapta[i] = t; // cupl t si i
            return 1;
        }
    }
    for(int i: lis[t])
    {
        if(cupleaza(dreapta[i]) != 0)
        {
            dreapta[i] = t;
            stanga[t] = i;
            return 1;
...
void graf :: cuplaj_maxim()
{
    int finishh = 0; // init cu 0
    while(!finishh) // cat timp este 0
    {
        finishh = 1;
        vizi_cuplaj.reset();
        for (int i = 1; i <= n; i++)
        {
            if (stanga[i] == 0 && cupleaza(i))
            {
                rasp_cuplaj++;
                finishh = 0;
            }
...
void graf :: creare_graf(int N, int M, int S)
{
    this-> N = N;
    this-> M = M;
    this-> S = S;
...
void graf :: creare_adiacente(int M)
{
    int nod1, nod2;
    for(int i = 1; i <= M; i++)
    {
...
void graf :: creare_graf_Dfs(int N, int M)
{
    this -> N = N;
    this -> M = M;
    ok = vector <int> (N+1);
...
void graf :: creare_adiacente_Dfs(int M)
{
    int nod1, nod2;
    for(int i = 1; i <= M; i++)
    {
...
void graf :: dfs_ctc2(int nod1)
{
    viz_t[nod1] = 1; //marcam in tabl viz_t nodul1
    rez[ctck].push_back(nod1); //retinem in dreptul nr de comp tari conexe indicativul nodului marcat
    for(auto nod2: G_t[nod1])
        if(!viz_t[nod2])
            dfs_ctc2(nod2);
}
void graf :: creare_graf_ctc(int N, int M)
{
    this -> N = N;
    this -> M = M;
}
void graf :: creare_adiacente_ctc(int M)
{
    for(int i = 1; i <= M; i++)
    {
        int nod1, nod2;
...
void graf :: fct_ctc(int N)
{
    ...
    while(!stiv.empty()) //cat timp mai avem elemente in stiva
    {
        if(viz_t[stiv.top()] == 0) //daca in viz_t nu a fost vizitat elem din vf-ul stivei
        {
            ...
            ctck=ctck+1; //increm nr de comp
        }
        stiv.pop();
    }
    g<<ctck<<"\n";
    for(int i = 0; i < ctck; i++)
    {
        for(auto it: rez[i])
            g<<it<<" ";
        g<<"\n";
...
void graf :: dfs_critical_connections(int nod)
{
...
void graf :: criticalConnections(int N, int M)
{
    tata.resize(N, -1);
    l.resize(N, -1);
    D_C.resize(N);
...
//---------------critical connections---------------------
//---------------componente biconexe----------------
void graf :: bic(int nod1, int nod2)
{
    vector<int> local;
...
void graf :: dfs_bic(int nod_curent)
{
    q++;
...
void graf :: creare_graf_bic(int N, int M)
{
    this -> N = N;
    this -> M = M;
}
void graf :: creare_adiacente_bic(int M)
{
    int nod1, nod2;
    for(int i = 1; i <= M; i++)
    {
...
//------------------componente biconexe---------------------
//------------------havel hakimi--------------------
bool graf :: HavelOK(int N, vector<int>&v)
{
    while(1)
    {
...
void graf :: afisare_sortare_topo()
{
    while(!s.empty())
    {
        int var = s.top();
...
void graf :: creare_graf_st(int N, int M)
{
    this -> N = N;
    this -> M = M;

}
void graf :: creare_adiacente_st(int M)
{
    int nod1, nod2;
    for(int i = 0; i < M; i++)
    {
